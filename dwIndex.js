...
newWallet : () => {
	...
	valorTxt.textContent = `₹${nWallet.blc}.00`;
	...
	rsvdValue.textContent = `₹${nWallet.rsvd}.00`;
	...
	valorTxt.textContent = `₹${nWallet.blc.toFixed(2)}`;
	});
	...
	valorTxt.textContent = `₹${nWallet.blc.toFixed(2)}`;
	rsvdValue.textContent = `₹${nWallet.rsvd.toFixed(2)}`;
	});
...
trashIcon.addEventListener('click', function () {
	if(nWallet.blc != 0) {
		alert(`You need to empty the account first. Available: ₹${nWallet.blc.toFixed(2)}`)
	} else {
